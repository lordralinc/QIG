...
class TextProcessor:
    ...
    def get_line_size_by_box(
        ...
        emojies = self.generator.emoji_source.get_emojies(text)

        for emoji in emojies:
            text = text.replace(emoji, "", 1)

        max_font_size = max_font_size or self.generator.max_font_size
        ...
        for size in range(max_font_size, 0, -1):
            ...
            text_size = Size(
                ...
                + math.floor(len(emojies) * size * self.generator.emoji_scale),
                height=size,
            )
    ...
    def draw_single_line(
        ...
        for chunk in self.generator.emoji_source.chunk_by_emoji(entity["content"]):
            if chunk["type"] == "emoji":
                emoji_image = self.generator.emoji_source.get_image(chunk["content"]).resize(
    ...
    def get_entities_size(
        self, entities: list[DrawEntity], max_box_size: SizeBox, max_font_size: int | None = None
    ) -> tuple[int, Size]:
        max_font_size = max_font_size or self.generator.max_font_size
        for size in (max_font_size, 0, -1):
            max_current_size = Size(0, size)
            current_position = Point(0, 0)
            for entity in entities:
                if entity["type"] == "emoji":
                    current_position = Point(
                        current_position.x + math.floor(size * self.generator.emoji_scale),
                        current_position.y,
                    )
                    ...
                    emojies = self.generator.emoji_source.get_emojies(content)
                    for emoji in emojies:
    ...
    def draw_entities(
        ...
        font_size, entities_size = self.get_entities_size(entities, box)
        ...
        for entity in entities:
            if entity["type"] == "emoji":
                emoji_image = (
                    ...
                    .resize(
                        (
                            math.floor(font_size * self.generator.emoji_scale),
                            math.floor(font_size * self.generator.emoji_scale),
                        ),
            ...
            if entity["type"] in (
                ...
                emoji_size = math.floor(font_size * self.generator.emoji_scale)
                for chunk in self.generator.emoji_source.chunk_by_emoji(entity["content"]):
